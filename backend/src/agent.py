...
class BaseJanSahayAgent(Agent):
    """Base class for Jan Sahay AI Agents with shared memory and safety tools."""
    ...
    async def record_caller_query(self, caller_query: str, domain: str = "general") -> str:
        ...
        caller_name = user_info.get("name", "Citizen")
        lang_pref = user_info.get("language_preference", "English")
...
class FraudSpecialist(BaseJanSahayAgent):
...
class GovernmentSchemeSpecialist(BaseJanSahayAgent):
...
class LoanSpecialist(BaseJanSahayAgent):
...
class AgriSpecialist(BaseJanSahayAgent):
...
class Assistant(BaseJanSahayAgent):
    """Main Jan Sahay Assistant handling general intake, welfare schemes, and handoffs to specialists."""
    ...
    async def handoff_to_specialist(
        ...
        user_info = db.get_user(self.user_id) or {}
        caller_name = user_info.get("name", "")
        language_pref = user_info.get("language_preference", "English")
        facts = user_info.get("facts", {})
        ...
        active_session = getattr(self, "_voice_session", None) or (getattr(self, "_activity", None).session if hasattr(self, "_activity") and self._activity else None)
        if active_session:
            try:
    ...
    async def handoff_to_main_guide(self, user_inquiry_summary: str = "") -> str:
        """Transfers the caller back to the main Jan Sahay Intake Guide (Anisha) for general scheme inquiries.
        ...
        db.update_call_progress(self.call_id, outcome_type="Connecting to Main Jan Sahay Guide (Anisha)...")
        ...
        if language_pref.lower() == "hindi":
            ...
            connecting_line = "I am connecting you back to our main Jan Sahay Guide, Anisha. Please hold on a moment."
            main_intro = "Hello! I am Anisha from Jan Sahay."
        ...
        db.update_call_progress(self.call_id, status="success", outcome_type="Connected: Main Jan Sahay Guide (Anisha)")
        logger.info("[CONNECTED] Main Jan Sahay Guide (Anisha) is now active.")
